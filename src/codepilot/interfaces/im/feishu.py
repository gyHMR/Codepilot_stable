...
logger = logging.getLogger("codepilot.im.feishu")
...
class FeishuAdapter:
    """Feishu adapter for events, replies, card updates, and metadata cache."""
    ...
    # ------------------------------------------------------------------
    # Send text messages, including replies in threads.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Update an already-sent message for streaming text effects.
    # ------------------------------------------------------------------

    def update_text(self, message_id: str, text: str) -> None:
        """Update an already-sent card message."""
        httpx = _import_httpx()
        token = self._get_tenant_access_token()
        url = f"{self.config.api_base}/im/v1/messages/{message_id}"
        headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json; charset=utf-8"}
        card = self._build_card_content("Codepilot", text)
        payload = {
            "content": card,
        }
    ...
    # ------------------------------------------------------------------
    # Send interactive card messages with Markdown content.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # User and channel metadata cache.
    # ------------------------------------------------------------------
    ...
    def _post_message(self, httpx, headers: dict, url: str, payload: dict) -> str | None:
        """Send a new Feishu message and return its message id."""
        with httpx.Client(timeout=self.config.request_timeout_seconds) as client:
            resp = client.post(url, headers=headers, json=payload)
            resp.raise_for_status()
            body = resp.json()
