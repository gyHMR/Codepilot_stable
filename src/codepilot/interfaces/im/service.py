...
"""Platform-neutral IM to Agent service layer.

Design notes:
1) Keep one long-lived AgentSession per channel.
2) Send a placeholder first, then patch streaming updates.
3) Include token usage and cost when available.
4) Inject global and channel MEMORY.md content.
"""
...
from codepilot.sessions.memory import load_merged_memory
...
logger = logging.getLogger("codepilot.im.service")
...
@dataclass
class _ChannelState:
    """Long-lived state for one channel."""
...
class IMService:
    ...
    def _get_or_create_channel_session(self, message: IMIncomingMessage) -> tuple[AgentSession, str]:
        """Get or create the long-lived session for a channel."""
        channel_key = self._channel_key(message)
    ...
    def _invalidate_channel_session(self, message: IMIncomingMessage) -> None:
        """Invalidate the channel session for control commands."""
        channel_key = self._channel_key(message)
    ...
    def _build_user_context(self, message: IMIncomingMessage) -> str:
        """Build prompt context from user and channel metadata."""
        parts: list[str] = []
    ...
    async def _prompt_with_streaming(
        ...
        """Run a streaming prompt with placeholder updates."""
    ...
    def _send_reply(self, message: IMIncomingMessage, text: str) -> None:
        """Send the final reply in non-streaming mode."""
    ...
    async def _handle_control_command(self, message: IMIncomingMessage) -> bool:
        ...
        if text in {"/clear", "/new"}:
            ...
            self.adapter.send_text(
                IMOutgoingText(
                    channel_id=message.channel_id,
                    text=f"Created a new session: `{new_session_id}`. Future messages will use the new context.",
                    thread_id=message.thread_id,
                    reply_to_message_id=message.message_id,
                )
            )
        ...
        if text == "/session":
            ...
            self.adapter.send_text(
                IMOutgoingText(
                    channel_id=message.channel_id,
                    text=f"Current session: `{session_id}`{cum}",
                    thread_id=message.thread_id,
                    reply_to_message_id=message.message_id,
                )
            )
