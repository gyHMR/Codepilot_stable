...
logger = logging.getLogger("codepilot.im.feishu.longconn")
...
def run_feishu_long_connection(service: IMService, options: FeishuLongConnOptions) -> None:
    """Run the Feishu SDK long-connection event loop."""
    ...
    def _on_p2_im_message_receive_v1(data: Any) -> None:
        """Handle lark-oapi P2ImMessageReceiveV1 callbacks."""
        try:
            raw = _to_dict(data)
...
def _run_async(coro: Any) -> None:
    """Schedule a coroutine in an existing loop or a new thread."""
    try:
...
def _parse_event_object(data: Any) -> IMIncomingMessage | None:
    """Extract a message from a strongly typed lark-oapi event object."""
...
def _parse_ws_message(payload: dict[str, Any]) -> IMIncomingMessage | None:
    """Parse a message from a raw websocket payload."""
...
def _strip_bot_mentions_from_objects(text: str, mentions: list[Any]) -> str:
    """Remove bot mention placeholders from typed mention objects."""
    for m in mentions:
...
def _strip_bot_mentions_from_dicts(text: str, mentions: list[dict[str, Any]]) -> str:
    """Remove bot mention placeholders from dict mention objects."""
    for m in mentions:
