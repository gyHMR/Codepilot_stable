...
"""Result redaction and output trust marking."""
...
from .authoring import AgentToolResult
        ...
        *,
        ...
        for block in result.content:
            if not isinstance(block, TextContent):
                continue
            guarded_text, block_redacted, block_findings = _redact_text(
                block.text,
            ...
            prompt_injection_suspected = (
                prompt_injection_suspected or bool(block_prompt_findings)
            )
...
def _preserve_workspace_source(metadata: ToolMetadata | None) -> bool:
    return (
        ...
        and metadata.name == "read"
        and metadata.category == "filesystem"
    )
