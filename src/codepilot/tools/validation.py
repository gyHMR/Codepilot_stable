...
def _validate_value(value: Any, schema: dict[str, Any], *, path: str) -> list[str]:
    ...
    if _should_validate_object(value, schema, expected_types):
    ...
    *,
    ...
    for name, property_schema in properties.items():
        ...
        if isinstance(property_schema, dict):
            errors.extend(
                _validate_value(value[name], property_schema, path=_child_path(path, name))
            )
    ...
    if additional is False:
        ...
        for name in unknown_names:
            errors.extend(
                _validate_value(value[name], additional, path=_child_path(path, name))
            )

    return errors


def _should_validate_object(
    value: Any,
    schema: dict[str, Any],
    expected_types: tuple[str, ...],
) -> bool:
    return _matches_json_type(value, "object") and (
...
def _format_types(types: tuple[str, ...]) -> str:
    return types[0] if len(types) == 1 else " or ".join(types)


def _child_path(path: str, name: object) -> str:
    return str(name) if path == "$" else f"{path}.{name}"
