...
def test_opening_session_restores_active_plan_and_context_checkpoint(tmp_path: Path) -> None:
    ...
    from codepilot.sessions.service import BeginRunRequest, CommitBoundaryRequest
    ...
    started = original.state_service.commit_boundary(
    ...
    original.state_service.commit_boundary(
        CommitBoundaryRequest(
            session_id=original.session_id,
            run_id="run_restore",
            status="waiting",
